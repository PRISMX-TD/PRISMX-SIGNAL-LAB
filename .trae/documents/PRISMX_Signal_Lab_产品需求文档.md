...
- 当前阶段目标：在本地用模拟（Demo）账户跑通完整链路，验证信号、下单、绑定、回执闭环，为后续多用户对外服务打基础。
...
| 管理员（本地阶段简化） | 预置账户 | 手动发布信号、查看所有用户连接状态 |
...
用户注册登录后，进入 EA 绑定页获取专属 API Token，并把 Token 填入自己 MT5 上挂载的 PRISMX EA。EA 携带 Token 连接平台完成认证与绑定。信号引擎按技术指标产出信号并通过 WebSocket 推送到信号面板。用户在面板点击「下单」并二次确认后，平台按 user_id 将指令精准投递给该用户的 EA，EA 在本地 MT5 执行下单，并把成交回执上报平台，平台落库后推送回网页更新订单状态。
...
- 使用 MT5 Demo 模拟账户，不接入真实资金。
- 后端地址通过配置切换，本地使用 localhost，便于后续平滑部署。
- 信号来源本阶段为技术指标算法（如均线/RSI 交叉），预留手动发布与外部信号源扩展位。
- 提供两个版本的 EA（均含中英双语界面/日志）：见技术架构文档。

## 6. 优化迭代记录

### 6.1 第一批：安全与正确性（本机阶段可做）

本批聚焦"不出错、不重复下单、看得准、用得稳"，不涉及真实行情接入与线上部署（留待后续）。
...
- 真实行情信号接入（用户后续自行对接）。
- 后端上线部署 + 正式数据库（当前本机 SQLite，多人/公网访问时再迁移）。
