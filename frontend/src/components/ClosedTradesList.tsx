              ...
              <tbody>
                {pageTrades.map((tr) => (
                  <tr key={tr.id} className="border-b border-white/5">
                    <td className="whitespace-nowrap px-3 py-2 text-neutral-400">{fmtTime(tr.closedAt)}</td>
                    ...
                    <td className="px-3 py-2 font-mono text-neutral-200">{tr.closeVolume}</td>
                    <td className="px-3 py-2 font-mono text-neutral-200">{tr.closePrice ?? '-'}</td>
              ...
              <div key={tr.id} className="py-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-sm font-semibold text-neutral-100">{displaySymbol(tr.symbol)}</span>
                    <span className={`tag ${tr.side === 'BUY' ? 'bg-up/15 text-up' : 'bg-down/15 text-down'}`}>
                      {tr.side === 'BUY' ? t('common.buy') : t('common.sell')}
                    </span>
                  </div>
                ...
                <div className="mt-0.5 text-right text-xs text-neutral-500">{fmtTime(tr.closedAt)}</div>
              </div>
            ))}
          </div>
