...
export default function PresetPicker({ options, loading, error, onStart, onCancel }: PresetPickerProps) {
  ...
  return (
    <section className="card glass stg-picker">
      <div className="stg-picker-head">
        <h3 className="font-display">{t('strategy.presetStartTitle')}</h3>
        <button type="button" onClick={onCancel} className="stg-act">{t('common.cancel')}</button>
      </div>
      ...
      {/* 推荐预设在前、空白在后：新手的默认路径是"挑一个看得懂的改"，而"从空白
          ...
          small grey ones. */}
      <h4>{t('strategy.presetPickTitle')}</h4>
      <p className="stg-picker-hint">{t('strategy.presetPickHint')}</p>
      {loading ? (
        <p className="stg-picker-note">{t('strategy.presetLoading')}</p>
      ) : error ? (
        <p className="stg-picker-note warn">{error}</p>
      ) : options.length === 0 ? (
        <p className="stg-picker-note">{t('strategy.presetNone')}</p>
      ) : (
        <div className="stg-presets">
          {options.map((o) => (
            <button key={o.key} type="button" onClick={() => onStart(o.key)} className="stg-preset">
              <span className="stg-preset-top">
                <b>{o.label}</b>
                <span>{t('strategy.presetUse')} →</span>
              </span>
              <p>{o.desc}</p>
            </button>
          ))}
      ...
      <div className="stg-blank">
        <div className="min-w-0">
          <b>{t('strategy.presetBlankTitle')}</b>
          <p>{t('strategy.presetBlankHint')}</p>
        </div>
        <button type="button" onClick={() => onStart(null)} className="stg-act">
