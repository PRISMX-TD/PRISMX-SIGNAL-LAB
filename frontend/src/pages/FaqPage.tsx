  ...
  return (
    <div className="relative min-h-[100dvh]">
      ...
      <main className="relative z-10 mx-auto max-w-3xl px-5 py-10 sm:py-14">
        ...
        <p className="mt-6 text-[15px] leading-relaxed text-neutral-300">{t('faqPage.intro')}</p>
        ...
        <nav className="mt-10 flex flex-wrap gap-x-6 gap-y-2 border-t border-white/[0.06] pt-6 text-sm">
          {(['terms', 'privacy', 'risk'] as const).map((d) => (
            <Link key={d} to={localePath(lang, `/${d}`)} className="text-neutral-400 transition hover:text-prism-300">
              {t(`legal.${d}.title`)}
            </Link>
          ))}
        </nav>
